...
describe('Export Campaign to CSV method', function () {
  ...
  it('should return a valid csv', function () {
    ...
    const expectedHeader = ['Name', 'Outlet', 'Title', 'Status', 'Latest Activity', 'Updated At', 'Updated By', 'Coverage', 'Owner']
    ...
    contactRows.forEach((row, i) => {
      ...
      assert.equal(row[7], 'No coverage yet')
      assert.equal(row[8], users[0].profile.name)
    })
